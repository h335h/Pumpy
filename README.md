...
## Описание
Бот собирает научные новости, статьи, анонсы конференций и вакансии из Telegram-каналов и RSS-лент ведущих журналов, фильтрует их по семантической близости к вашему профилю интересов и отправляет дайджест в личные сообщения ВКонтакте.
...
- **Множество источников** – Telegram-каналы, RSS журналов (Nature, ASM, APS, bioRxiv).
- **Безопасность** – все токены и персональные данные хранятся в `.env`.
- **Легковесность** – работает на MacBook Air M1 с 8 ГБ RAM.

## Как это работает
1. **Сбор** (`collectors/rss_collector.py`, `collectors/telegram_collector.py`) – получает посты из Telegram и RSS, сохраняет в SQLite.
